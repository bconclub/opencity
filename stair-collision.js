...
export function createStairCollision(architecture,{radius=.78,halfLength=.7,includeLanding=false,getVehicleFootprint}={}){
 ...
 return{footprint:footprint.map(p=>Object.freeze(p)),contains,collide(x,y,headingDegrees){if(![x,y,headingDegrees].every(Number.isFinite))return null;const body=getVehicleFootprint?.();if(body)return convexFootprintContact(vehicleFootprintCorners(body,x,y,headingDegrees),footprint);const angle=headingDegrees*Math.PI/180;for(const d of [-halfLength,halfLength]){const contact=probe(x+Math.sin(angle)*d,y+Math.cos(angle)*d);if(contact)return contact;}return null;}};
}
