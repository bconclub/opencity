...
export function loadAutoAsset(){
 if(!assetPromise)assetPromise=import('https://unpkg.com/three@0.169.0/examples/jsm/loaders/GLTFLoader.js').then(({GLTFLoader})=>new GLTFLoader().loadAsync('./assets/auto/auto-rickshaw.glb')).catch(error=>{assetPromise=null;throw error;});
 return assetPromise;
}
export function upgradeAuto(T,model,remote){
 ...
 model.ready=loadAutoAsset().then(gltf=>{
  ...
  model.body.add(imported);model.group.userData.assetStatus='ready';model.group.userData.assetSource='user-auto-rickshaw';if(paintKey)apply(paintKey);window.dispatchEvent(new CustomEvent('vehicle-asset-ready',{detail:{vehicle:'auto'}}));return true;
