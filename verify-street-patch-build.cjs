...
for(const m of source){
 ...
 const key=JSON.stringify(m.color);if(!groups.has(key))groups.set(key,{color:m.color,positions:[],normals:[],indices:[]});const g=groups.get(key),base=g.positions.length/3;
 ...
 for(let i=0;i<m.indices.length;i+=3){const ids=[m.indices[i],m.indices[i+2],m.indices[i+1]];const triangleKey=ids.map(n=>m.positions.slice(n*3,n*3+3).join(',')).sort().join('|');if(seenTriangles.has(triangleKey)){duplicatesRemoved++;continue;}seenTriangles.add(triangleKey);g.indices.push(...ids.map(n=>n+base));const p=ids.map(n=>[m.positions[n*3],m.positions[n*3+2]]);if(Math.abs((p[1][0]-p[0][0])*(p[2][1]-p[0][1])-(p[2][0]-p[0][0])*(p[1][1]-p[0][1]))<1e-8)continue;const ring=p.map(v=>lnglat(...v));ring.push(ring[0]);footprint.features.push({type:'Feature',properties:{},geometry:{type:'Polygon',coordinates:[ring]}});}
}
...
for(const g of groups.values()){const material=doc.materials.push({pbrMetallicRoughness:{baseColorFactor:[...g.color.map(c=>c<=.04045?c/12.92:Math.pow((c+.055)/1.055,2.4)),1],metallicFactor:0,roughnessFactor:.95},doubleSided:true})-1;const mesh=doc.meshes.push({primitives:[{attributes:{POSITION:attr(g.positions,'VEC3'),NORMAL:attr(g.normals,'VEC3')},indices:attr(g.indices,'SCALAR',true),material}]})-1;doc.scenes[0].nodes.push(doc.nodes.push({mesh})-1);}
