...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true,args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 ...
 for(const id of ['cycle','helicopter','auto','cybertruck','cybercab','kitt']){const data=await page.evaluate(id=>renderVehiclePreview(id),id);const file='assets/vehicles/previews/'+id+'.webp';fs.writeFileSync(file,Buffer.from(data.split(',')[1],'base64'));console.log(id,fs.statSync(file).size);}
