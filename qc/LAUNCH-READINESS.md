...
| Portico, stairs and coherent building heights | Revised full-scene candidate passes native performance, remains unpromoted | `vidhana-architecture-integration-report.md`; four poses worst +3.49%, one fewer draw batch. Stair moire still visible; filtering investigation underway. Missing upper pedestal/emblem explicit; actual summit39.8m. |
| Vidhana passage and stair collision | Isolated CPU candidate passes; browser integration underway | `vidhana-collision-integration-review.md`; mapped underpass has model clearance,58 old false-positive collisions opened,31,314 other outcomes preserved. Automatic private routing requires real exclusion. |
| Landmark focus metadata | Isolated source-data and unit tests pass | `landmark-focus-review.md`; hidden district core previously beat visible crown. Candidate filters non-rendered features, keeps stable IDs, reports measured display height without changing OSM data. Browser dwell check remains. |
