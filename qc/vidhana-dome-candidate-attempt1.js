...
export function buildLandmarks(T,data,xy){
 ...
 for(let i=0;i<ring.length;i++){
  ...
  for(let j=0;j<3;j++){
   const left=j*length/3,right=(j+1)*length/3,mid=(left+right)/2,w=(right-left)*.44,l=mid-w/2,r=mid+w/2,z0=D.lowerCourseTop+.30,z1=D.galleryTop-.32;
   ...
   const w0=.15,w1=.24,p=[at(mid-w0,25.4,.01),at(mid+w0,25.4,.01),at(mid-w1,27.35,.72),at(mid+w1,27.35,.72),at(mid-w1,27.35,.01),at(mid+w1,27.35,.01)];
 ...
 for(let i=0;i<48;i++){const a=i*Math.PI*2/48,x=dc[0]+Math.cos(a)*rx*1.04,y=dc[1]+Math.sin(a)*ry*1.04;append(new T.BoxGeometry(.11,.11,.85),stone,new T.Matrix4().makeTranslation(x,y,28.625));}
 lathe([[1.03,29],[1.05,29],[1.05,29.09],[1.03,29.09],[1.03,29]]);
 ...
 model.architecture={...model.architecture,domeUpdate:{...D,mappedCenter:dc,mappedDiameter:[rx*2,ry*2],drumCorners:ring.length,bays,brackets,addedTriangles:added,otherSixDomesUnchanged:true,coreRemoved:true,reviewOnly:true}};
 return model;
}
