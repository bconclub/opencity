...
import {retainTrafficLoops,applyTrafficDirections,routeState,advanceTrafficRoute,clonePath,tickTraffic,mappedControls,trafficSpawnSafe,vehicleContact} from '../traffic-simulation.js';
...
const report={created:new Date().toISOString(),sourceHashes:Object.fromEntries(['traffic-simulation.js','npc-traffic.js','auto-roads.js','driving-data.js','district-data.json','vidhana-road-network.json','assets/streets/furniture.json'].map(p=>[p,crypto.createHash('sha256').update(read(p)).digest('hex')])),sourceFeatureCount:source.features.length,verifiedFeatureCount:data.features.length,scenarios:[]};
