...
export function createDrivingWorld(data,graph,{passage}={}){
 ...
 for(const f of data.features){if(f.properties._layer!=='building'||Number(f.properties.render_min_height)>2.2)continue;const r=f.geometry.coordinates.map(r=>r.map(toLocal));const b=[Math.min(...r[0].map(p=>p[0]))-2,Math.min(...r[0].map(p=>p[1]))-2,Math.max(...r[0].map(p=>p[0]))+2,Math.max(...r[0].map(p=>p[1]))+2];r.sourceFeature=f;insert(cells,b,r);}
 for(const e of graph.edges){const a=graph.nodes[e.a].p,b=graph.nodes[e.b].p,w=e.width?e.width/2:({service:2.5,minor:3.5,tertiary:5,secondary:6,primary:7,trunk:8}[e.kind]||3.5);insert(roads,[Math.min(a[0],b[0])-w,Math.min(a[1],b[1])-w,Math.max(a[0],b[0])+w,Math.max(a[1],b[1])+w],{a,b,w});}
 ...
 return{onRoad(x,y){return at(roads,x,y).some(r=>{const q=nearest([x,y],r.a,r.b);return Math.hypot(x-q[0],y-q[1])<r.w;});},collide(x,y,heading){if(x<lo[0]+2)return{x:1,y:0};if(x>hi[0]-2)return{x:-1,y:0};if(y<lo[1]+2)return{x:0,y:1};if(y>hi[1]-2)return{x:0,y:-1};const a=heading*Math.PI/180;
  for(const offset of [-.7,.7]){const p=[x+Math.sin(a)*offset,y+Math.cos(a)*offset];for(const rings of at(cells,...p)){if(passage?.allows(rings.sourceFeature,p,{x,y,heading}))continue;const within=inside(p,rings[0])&&!rings.slice(1).some(r=>inside(p,r));let best=Infinity,q;for(const r of rings)for(let i=1;i<r.length;i++){const n=nearest(p,r[i-1],r[i]),d=Math.hypot(p[0]-n[0],p[1]-n[1]);if(d<best){best=d;q=n;}}if(within||best<.78){const sign=within?-1:1;return{x:sign*(p[0]-q[0])/(best||1),y:sign*(p[1]-q[1])/(best||1)};}}}return null;
 }};
}
