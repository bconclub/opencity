...
document.getElementById('run').onclick=async()=>{
 ...
 pointer('pointerdown',4,right,y);assert(!stick().hidden&&Math.abs(parseFloat(stick().style.left)-right)<.01,'Right touch creates pad at finger');pointer('pointermove',4,right-20,y-30);assert(state().analog.throttle>0&&state().analog.steer<0,'Right-origin drive and steer');pointer('pointerup',4,right-20,y-30);assert(zeros()&&stick().hidden,'Right-origin release clears input');
