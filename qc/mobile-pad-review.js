...
document.getElementById('run').onclick=async()=>{
 ...
 pointer('pointerdown',4,right,y);assert(!stick().hidden&&Math.abs(parseFloat(stick().style.left)-right)<.01,'Right touch creates pad at finger');pointer('pointermove',4,right-20,y-30);assert(state().analog.throttle>0&&state().analog.steer<0,'Right-origin drag drives and steers');pointer('pointerup',4,right-20,y-30);assert(zeros()&&stick().hidden,'Right-origin release clears input');
 for(const [id,px,py] of [[41,innerWidth/2,y],[42,8,8],[43,innerWidth-8,innerHeight-8]]){pointer('pointerdown',id,px,py);assert(!stick().hidden&&Math.abs(parseFloat(stick().style.left)-px)<.01&&Math.abs(parseFloat(stick().style.top)-py)<.01,'Pad anchors at centre/edge '+id);pointer('pointerup',id,px,py);}
 document.getElementById('preview').dispatchEvent(new PointerEvent('pointerdown',{pointerId:44,pointerType:'touch',bubbles:true,cancelable:true}));assert(stick().hidden&&zeros(),'Menu/button touch does not start movement');
 mode='flight';pointer('pointerdown',5,right,y);pointer('pointermove',5,right+20,y-30);assert(state().analog.steer>0&&state().analog.steer<1&&state().keys.length===0,'Helicopter uses analog pad, no full-lock arrow keys');
