...
const changed=['npc-traffic.js','traffic-simulation.js','blender-vehicle.js'];
(async()=>{
 const baseline=Object.fromEntries(await Promise.all(changed.map(async f=>{const r=await fetch('https://www.opencity.world/'+f+'?benchmark=0.0.28');if(!r.ok)throw Error('Baseline fetch '+f);return[f,await r.text()];})));
 ...
 fs.writeFileSync('qc/release-29-lighting-performance.json',JSON.stringify({device:'Same Windows host, Edge headless SwiftShader software renderer',viewport:[1100,760],results},null,2));
