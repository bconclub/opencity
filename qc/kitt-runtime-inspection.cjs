...
const defaultPaintOnly=process.argv.includes('--default-paint-only'),reportName=defaultPaintOnly?'kitt-runtime-default-paint-browser.json':'kitt-runtime-inspection-results.json';
if(!process.argv.includes('--run-gpu')){console.log('Prepared only. After GPU slot release: node qc/kitt-runtime-inspection.cjs --run-gpu');process.exit(0);}
...
const preservedBaseline=path.join(ROOT,'assets/vehicles/kitt-review/runtime-before-promotion.glb');
const assets={before:fs.readFileSync(process.env.KITT_BASELINE_ASSET||(fs.existsSync(preservedBaseline)?preservedBaseline:path.join(ROOT,'assets/vehicles/kitt.glb'))),candidate:fs.readFileSync(process.env.KITT_CANDIDATE_ASSET||path.join(ROOT,'assets/vehicles/kitt-review/kitt-reference.glb'))};
const hash=bytes=>crypto.createHash('sha256').update(bytes).digest('hex');
if(!defaultPaintOnly)assert.notEqual(hash(assets.before),hash(assets.candidate),'Before/candidate bytes identical: specify an actual baseline GLB through KITT_BASELINE_ASSET.');
...
const blender=instrument(fs.readFileSync(path.join(ROOT,'blender-vehicle.js'),'utf8'),' return{group,body,front,wheels,get wheelRadius',` (window.__kittRuntimeModels??=[]).push({id,group,body,wheels,ready,setPaint,get rig(){return rig;},get scanners(){return scanners;}});\n`);
const remote=instrument(fs.readFileSync(path.join(ROOT,'multiplayer-render.js'),'utf8'),' const origin=maplibregl.MercatorCoordinate.fromLngLat',` window.__kittRuntimeRemoteEntries=entries;\n`);
const tuning=fs.readFileSync(path.join(ROOT,'vehicle-tuning.js'),'utf8').replace(/kitt:\{([^}]+)\}/,(_,body)=>'kitt:{'+body.replace(/wheelbase:[\d.]+/,'wheelbase:2.5654').replace(/wheelRadius:[\d.]+/,'wheelRadius:.324')+'}');
assert(tuning.includes('wheelbase:2.5654'),'KITT tuning interception failed');
const report={status:'RUNNING',base,started:new Date().toISOString(),candidatePhysicsOverride:{wheelbase:2.5654,wheelRadius:.324},variants:[],errors:[],note:'Full app with local actual room server. Only GLB response/tuning and read-only inspection hooks are intercepted; production files remain untouched. GPU visuals require human review.'};
...
async function snapshot(page){return page.evaluate(()=>{
 const models=(window.__kittRuntimeModels||[]).filter(m=>m.id==='kitt'&&m.group.parent).map(m=>{
  const entry=[...(window.__kittRuntimeRemoteEntries||new Map())].find(([,e])=>e.model?.group===m.group),materials=new Map();
...
async function startKitt(page,index=0){await page.locator('[data-ride=kitt]').click();const departures=page.locator('.departure-options button');await departures.first().waitFor();await departures.nth(Math.min(index,(await departures.count())-1)).click();await page.waitForFunction(()=>window.autoState?.().active&&autoState().vehicleType==='kitt');}
  ...
  for(const variant of defaultPaintOnly?['candidate']:['before','candidate']){
   ...
   try{
    for(const who of ['driver','observer']){
     const context=await browser.newContext({viewport:{width:1100,height:800},serviceWorkers:'block'});contexts.push(context);const p=await context.newPage();pages.push(p);p.setDefaultTimeout(15000);p.setDefaultNavigationTimeout(90000);
     ...
     await p.route('**/assets/vehicles/kitt.glb',r=>{run.requests.push({who,assetSHA256:hash(assets[variant]),url:r.request().url()});return r.fulfill({contentType:'model/gltf-binary',body:assets[variant]});});
    ...
    stage('driver city loading and room creation');await driver.goto(base);const entry=driver.locator('.multiplayer-name-dialog');await entry.waitFor({state:'visible',timeout:90000});await entry.getByLabel('Player name',{exact:true}).fill('KITT Driver');await entry.getByRole('button',{name:'Create room',exact:true}).click();await driver.waitForFunction(()=>multiplayerState().connected);const room=await driver.evaluate(()=>multiplayerState().room);
    stage('observer city loading and room join');await observer.goto(base);const join=observer.locator('.multiplayer-name-dialog');await join.waitFor({state:'visible',timeout:90000});await join.getByLabel('Player name',{exact:true}).fill('KITT Observer');await join.getByRole('button',{name:'Join room',exact:true}).click();await join.locator('.multiplayer-entry-room input').fill(room);await join.getByRole('button',{name:'Join and start',exact:true}).click();await observer.waitForFunction(()=>multiplayerState().connected&&multiplayerState().players.length===2);
    stage('KITT departure and parked comparison');
    await driver.bringToFront();await startKitt(driver);await observer.waitForFunction(()=>(window.__kittRuntimeModels||[]).some(m=>m.id==='kitt'&&m.rig&&m.group.parent));await resume(driver);
    await waitPaint(driver,'local','303b3e');await waitPaint(observer,'remote','303b3e');
    ...
    assert.deepEqual(model(run.parkedLocal,'local').paint.map(m=>m.color),model(run.parkedRemote,'remote').paint.map(m=>m.color),'initial local and remote KITT paint differs');run.checks.push('no stored choice: initial local/remote KITT share legacy black palette before paint events');
    if(defaultPaintOnly){
     ...
     for(const [who,p] of [['driver',driver],['observer',observer]]){
      ...
      const file=`qc/kitt-runtime-default-paint-${who}.png`;await p.screenshot({path:path.join(ROOT,file),timeout:15000});run.screenshots.push(file);
     }
     run.status='PASS';stage('default paint check complete; full controls intentionally skipped');continue;
    }
    if(variant==='candidate'){assert(Math.abs(model(run.parkedLocal,'local').wheelbase-2.5654)<1e-5);assert(model(run.parkedRemote,'remote').scanners.every(s=>s.emissiveMap));}
    run.parkedPose=await frameObserver(observer,driver);
    for(const [who,p] of [['driver',driver],['observer',observer]]){const file=`qc/kitt-runtime-${variant}-parked-${who}.png`;await p.screenshot({path:path.join(ROOT,file)});run.screenshots.push(file);}
    stage('local and remote scanner animation');
    const localBefore=model(await snapshot(driver),'local'),remoteBefore=model(await snapshot(observer),'remote');
    await scannerChanged(driver,'local',localBefore.scanners.map(s=>s.intensity));await scannerChanged(observer,'remote',remoteBefore.scanners.map(s=>s.intensity));run.checks.push('local and remote eight-segment scanner changes through actual app render loop');
    ...
    try{await driver.waitForFunction(()=>autoState().speed>2,null,{timeout:15000});await driver.keyboard.down('ArrowRight');try{await driver.waitForFunction(()=>autoState().physics.steer>.08,null,{timeout:10000});await observer.waitForFunction(()=>(window.__kittRuntimeModels||[]).some(m=>m.id==='kitt'&&m.rig&&m.rig.steering.some(s=>s.tag[0]==='F'&&Math.abs(s.pivot.quaternion.y)>.005)),null,{timeout:5000});run.manualLocal=await snapshot(driver);run.manualRemote=await snapshot(observer);}finally{await driver.keyboard.up('ArrowRight');}}finally{await driver.keyboard.up('ArrowUp');}
    ...
    await observer.waitForFunction(before=>(window.__kittRuntimeModels||[]).some(m=>m.id==='kitt'&&m.rig&&m.wheels.some((w,i)=>w.quaternion.toArray().some((v,k)=>Math.abs(v-before[i][k])>1e-4))),quaternions(remoteBefore),{timeout:12000});
    run.manualRemote=await snapshot(observer);run.checks.push('manual throttle/steering and actual remote wheel motion');
    ...
    await driver.keyboard.down('ArrowDown');try{await driver.waitForFunction(()=>autoState().speed<-.5,null,{timeout:12000});await observer.waitForFunction(()=>multiplayerState().players.some(p=>p.name==='KITT Driver'&&p.pose.speed<-.2),null,{timeout:8000});run.reverse=await snapshot(driver);}finally{await driver.keyboard.up('ArrowDown');}run.checks.push('reverse speed reaches actual remote snapshot');
    ...
    const trimBefore=localBefore.nonPaint.map(m=>[m.name,m.color]),trimAfter=model(run.paintedDriver,'local').nonPaint.map(m=>[m.name,m.color]);assert.deepEqual(trimAfter,trimBefore,'paint changed trim/glass/lamp base colors');
    stage('second KITT client and independent paint');await observer.bringToFront();await startKitt(observer,1);await paint(observer,'red');await waitPaint(observer,'local','cf493a');await waitPaint(observer,'remote','397cce');await waitPaint(driver,'remote','cf493a');await pause(observer);
    run.twoKittDriver=await snapshot(driver);run.twoKittObserver=await snapshot(observer);assert.equal(run.twoKittObserver.models.length,2);assert(run.twoKittObserver.network.players.every(p=>p.pose.vehicle==='kitt'));run.checks.push('two actual clients use unchanged kitt ID, same selected bytes, independent blue/red local and remote paint');
    await frameObserver(observer,driver);const file=`qc/kitt-runtime-${variant}-two-client-painted.png`;await observer.screenshot({path:path.join(ROOT,file)});run.screenshots.push(file);
    ...
    for(let i=0;i<pages.length;i++){const p=pages[i],who=i?'observer':'driver',file=`qc/kitt-runtime-${defaultPaintOnly?'default-paint':variant}-error-${who}.png`;try{await p.screenshot({path:path.join(ROOT,file),timeout:8000});run.screenshots.push(file);}catch(e){(run.diagnosticErrors??=[]).push({who,error:String(e)});}}
