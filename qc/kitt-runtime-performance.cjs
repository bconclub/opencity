...
const crypto=require('node:crypto'),oldAsset='assets/vehicles/kitt-review/runtime-before-promotion.glb';
const source={baseline:fs.readFileSync(process.env.KITT_BASELINE_ASSET||(fs.existsSync(oldAsset)?oldAsset:'assets/vehicles/kitt.glb')),candidate:fs.readFileSync('assets/vehicles/kitt-review/kitt-reference.glb')},runs=[];
const hashes=Object.fromEntries(Object.entries(source).map(([key,bytes])=>[key,crypto.createHash('sha256').update(bytes).digest('hex')]));
assert.notEqual(hashes.baseline,hashes.candidate,'Performance comparison needs distinct baseline and candidate assets.');
...
for(const [index,mode] of ['baseline','candidate','candidate','baseline'].entries()){
 console.log('KITT frame gate '+index+' '+mode);
 ...
 await page.route('**/assets/vehicles/kitt.glb',r=>r.fulfill({contentType:'model/gltf-binary',body:source[mode]}));
 await page.goto('http://127.0.0.1:4173/');await page.locator('[data-ride=kitt]').click();await page.locator('.departure-options button').first().click();await page.waitForFunction(()=>window.autoState?.().active&&autoState().vehicleType==='kitt');
 ...
 await page.screenshot({path:`qc/kitt-runtime-performance-${index}-${mode}.png`});assert.deepEqual(errors,[]);assert(sample.layers.Auto?.calls>0);runs.push({index,mode,...sample,assetBytes:source[mode].length,errors});console.log(JSON.stringify({mode,meanMs:sample.meanMs,calls:sample.calls,triangles:sample.triangles}));await page.close();
}
}finally{await browser.close();const mean=mode=>{const values=runs.filter(r=>r.mode===mode);return values.reduce((n,r)=>n+r.meanMs,0)/values.length;},regression=(mean('candidate')/mean('baseline')-1)*100;const report={scope:'Full city static KITT chase view,1100x800,Edge SwiftShader,60frames/run,ABBA. Relative single-device gate, not physical phone FPS or multiplayer cost.',runs,baselineMeanMs:mean('baseline'),candidateMeanMs:mean('candidate'),regressionPercent:regression,passed:runs.length===4&&regression<=10};fs.writeFileSync('qc/kitt-runtime-performance.json',JSON.stringify(report,null,2)+'\n');if(runs.length===4)assert(regression<=10,'KITT full-scene frame time regressed more than10%');}})().catch(e=>{console.error(e);process.exitCode=1});
