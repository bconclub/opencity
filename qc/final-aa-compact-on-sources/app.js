...
async function boot(){
 try{
  ...
  let renderQuality={antialias:false,reason:'policy-unavailable'};
  try{renderQuality=(await import('./render-quality.js')).chooseRenderQuality();}catch(error){console.warn('Render quality probe unavailable; using compatible rendering.',error);}
  map=new maplibregl.Map({container:'map',canvasContextAttributes:{antialias:renderQuality.antialias,powerPreference:'high-performance'},style,center:[77.596,12.9716],zoom:16,pitch:60,bearing:-25,maxPitch:85,minZoom:13.7,maxZoom:20,maxBounds:[[77.585,12.966],[77.604,12.985]],hash:true,attributionControl:false});
