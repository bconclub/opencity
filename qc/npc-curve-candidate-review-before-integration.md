...
Candidate ready for review, not integrated into runtime. No graph, geometry, control phase, route selection, collision priority or live file changed.
...
This is a same-machine CPU comparison, not a GPU frame-time benchmark. Moving fleets diverge after braking, so the small measured difference includes changed proximity/collision work and timing noise. Browser appearance and frame-time checks still belong to the integration gate. Runtime remains frozen for the parent's current visual/performance captures.
