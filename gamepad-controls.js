...
export function decodePad(p){
 ...
 return{boost:b(5)>.5,turn:clamp(stick(p.axes[0])+b(15)-b(14)),forward:clamp(-stick(p.axes[1])+b(12)-b(13)),throttle:b(7)<.08?0:b(7),reverse:b(6)<.08?0:b(6),vertical:(b(7)<.08?0:b(7))-(b(6)<.08?0:b(6)),brake:b(0)>.5,camera:b(3)>.5,pause:b(9)>.5,takeoff:b(2)>.5};
}
export function createGamepadControls(parent,mode){
 ...
 const zero={boost:false,turn:0,forward:0,throttle:0,reverse:0,vertical:0,brake:false,camera:false,pause:false,takeoff:false};
 ...
 function poll(){
  ...
  const text=p.id+' · '+help+' · RB/R1 boost · Y/△ view · Start/Options pause';if(status.textContent!==text)status.textContent=text;
  return output;
 }
