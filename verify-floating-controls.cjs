...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true,args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 const p=await browser.newPage({viewport:{width:500,height:900},isMobile:true,hasTouch:true,serviceWorkers:'block'}),errors=[];p.on('pageerror',e=>errors.push(e.message));await p.route('**/multiplayer-client.js',r=>r.fulfill({contentType:'application/javascript',body:''}));await p.goto('http://127.0.0.1:4173/?floatingqc=1');await p.waitForFunction(()=>window.domeState?.().loaded,null,{timeout:120000});await p.locator('#mobile-menu-toggle').click();await p.locator('[data-tool=rides]').click();await p.locator('[data-ride=auto]').click();await p.waitForFunction(()=>window.autoState?.().active);await p.waitForTimeout(250);await p.locator('#mobile-drive-demo button').click();
 ...
 await p.evaluate(()=>document.getElementById('exit-auto').click());await p.locator('#mobile-menu-toggle').click();await p.locator('[data-tool=rides]').click();await p.locator('[data-ride=helicopter]').click();await p.waitForFunction(()=>window.flightState?.().active);await p.waitForTimeout(250);
 await touch('touchStart',[[4,380,550]]);await touch('touchMove',[[4,425,550]]);await p.waitForTimeout(300);assert((await p.evaluate(()=>flightState())).cameraOrbitYaw>1);await touch('touchEnd',[]);
 ...
 assert.equal(errors.length,0,errors.join('\n'));console.log(JSON.stringify({passed:true,twoTouches:active.touch,cameraYaw:active.auto.cameraOrbit.yaw,releaseCancelBlur:true,helicopterOrbit:true,doubleTapHover:true,errors}));
