...
async function installRenderer(map) {
 ...
 const clip=new T.Vector4(), RAD=Math.PI/180;
 ...
 function remove(id){const e=entries.get(id);if(!e)return;disposeModel(e.model);e.label.remove();entries.delete(id);}
 function valid(p){return p&&['auto','helicopter'].includes(p.vehicle)&&['lng','lat','altitude','heading','pitch','roll','speed'].every(k=>Number.isFinite(p[k]))&&Math.abs(p.lng)<=180&&Math.abs(p.lat)<85&&p.altitude>=-10&&p.altitude<10000;}
 function position(p){const c=maplibregl.MercatorCoordinate.fromLngLat([p.lng,p.lat],p.vehicle==='helicopter'?p.altitude+2:.02);return new T.Vector3((c.x-origin.x)/scale,(origin.y-c.y)/scale,c.z/scale);}
 function receive(event){
  ...
  for(const player of (Array.isArray(detail.players)?detail.players:[]).slice(0,32)){
   ...
   if(!e){
    ...
    labels.append(label);
    const model=own?null:(p.vehicle==='auto'?createAuto(T):createHelicopter(T));if(model)scene.add(model.group);
 ...
 const layer={id:'multiplayer-vehicles',type:'custom',renderingMode:'3d',
  ...
  render(gl,args){
   ...
   for(const e of entries.values()){
    ...
    e.point.copy(position(p));e.angle+=dt*(e.vehicle==='auto'?p.speed/.31:21);
    if(e.model){const m=e.model;m.group.position.copy(e.point);m.group.rotation.z=-p.heading*RAD;
     if(e.vehicle==='auto'){m.body.rotation.set(p.pitch,p.roll,0);m.wheels.forEach(wheel=>wheel.rotation.x=-e.angle);}
    ...
    clip.set(e.point.x,e.point.y,e.point.z+(e.vehicle==='helicopter'?6:2.7),1).applyMatrix4(camera.projectionMatrix);
   ...
   for(const [id,e] of [...entries].sort(([a],[b])=>String(a).localeCompare(String(b)))){
    ...
    const base=Math.max(margin,Math.min(h-height-margin,anchor.y-height));
    const overlaps=top=>placed.some(r=>left<r.right+gap&&left+width>r.left-gap&&top<r.bottom+gap&&top+height>r.top-gap);
    const candidates=[base];
    for(let step=1;step<=entries.size;step++)candidates.push(base-step*(height+gap),base+step*(height+gap));
 ...
 function destroy(){if(disposed)return;disposed=true;window.removeEventListener('multiplayer-players',receive);map.off('remove',destroy);for(const id of entries.keys())remove(id);labels.remove();if(map.getLayer(layer.id))map.removeLayer(layer.id);delete map.__multiplayerRenderer;}
 window.addEventListener('multiplayer-players',receive);map.on('remove',destroy);map.addLayer(layer);
 window.multiplayerRenderState=()=>({ownId,remoteCount:[...entries.values()].filter(e=>!e.own).length,tagCount:entries.size,players:[...entries].map(([id,e])=>({id,own:e.own,vehicle:e.vehicle,...e.current,position:e.point.toArray(),name:e.label.textContent,labelVisible:e.labelVisible,rotorAngle:e.angle})),disposed});
