...
export function createAuto(T,{remote=false}={}){
 ...
 for(const side of [-1,1]){
  ...
  const g=new T.ExtrudeGeometry(panel,{depth:.055,bevelEnabled:false,curveSegments:8});const m=mesh(g,paint,side*.72,0,0);m.rotation.set(Math.PI/2,Math.PI/2,0); // local x -> world y, local y -> world z
