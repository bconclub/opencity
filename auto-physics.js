...
export function createCar(x=0,y=0,heading=0){return{x,y,heading,vx:0,vy:0,steer:0,yaw:0,pitch:0,roll:0,pitchRate:0,rollRate:0,speed:0,distance:0,wheel:0,reverseWait:0,impacts:0,surface:'road'};}
export function advanceCar(s,input,dt,world={}){
 ...
 for(let i=0;i<steps;i++){
  ...
  let accel=input.throttle&&!brake?(input.boost?(p.boostAccel??8.4):(p.accel??4.2))*clamp(Number(input.throttle),0,1):input.reverse&&!brake&&(s.reverseWait>.35||forward<-.2)?-2.4*clamp(Number(input.reverse),0,1):0;
  ...
  forward=clamp(forward,-(p.reverseSpeed??4),input.boost?(p.boostSpeed??32):(p.maxSpeed??19));
  ...
  s.yaw+=(clamp(targetYaw,-limit,limit)-s.yaw)*(1-Math.exp(-h*5));
  ...
  else{const moved=Math.hypot(nx-s.x,ny-s.y);s.distance+=moved;s.wheel+=Math.sign(forward)*moved/(p.wheelRadius??.31);s.x=nx;s.y=ny;}
