...
export function advanceCar(s,input,dt,world={}){
 ...
 for(let i=0;i<steps;i++){
  ...
  else{const moved=Math.hypot(nx-s.x,ny-s.y);s.distance+=moved;s.wheel+=Math.sign(forward)*moved/.31;s.x=nx;s.y=ny;}
  const pitchTarget=clamp((forward-oldForward)/Math.max(h,.0001)*.007,-.08,.08),rollTarget=clamp(-forward*s.yaw*.018,-.14,.14);
